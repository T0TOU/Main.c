...
int main()
{
	...
	char a1 = 1;
	printf("%u\n", sizeof(a1));	//%u 是打印无符号整数的（unsigned）
	printf("%u\n", sizeof(+a1));//+c c经历了整形提升，所以它是一个int 所占的是4个字节Byte
	printf("%u\n", sizeof(!a1));//！是逻辑非运算符，任何非零值都被视为真，取反'!’变成0，还是char类型所以还是一个字节Byte
	printf("%d\n", !a1);
	return 0;
}